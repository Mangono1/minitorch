...
int main() {
    try {
        std::cout
            << "====================================\n"
            << " MiniTorch V0.6 Vulkan MULTIPLY Test\n"
            << "====================================\n\n";
        ...
        minitorch::VulkanComputePipeline pipeline(
            context,
            "shaders/vector_multiply.comp.spv",
            3
        );

        check(
            pipeline.valid(),
            "Vulkan multiply pipeline invalid"
        );

        std::cout << "MULTIPLY pipeline: PASS\n";
        ...
        const std::vector<float> expected{
            10.0f, 40.0f, 90.0f, 160.0f,
            250.0f, 360.0f, 490.0f, 640.0f
        };
